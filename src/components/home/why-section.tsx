...
export function WhySection() {
  const t = useTranslations("home.whySection");
  const tData = useTranslations("whyData.items");
            ...
            return (
              <Reveal key={item.id} delay={i * 0.05}>
                <div className="flex h-full flex-col gap-4 rounded-2xl p-2">
                  ...
                  <h3 className="text-[15px] font-semibold text-foreground">{tData(`${item.id}.title`)}</h3>
                  <p className="text-sm leading-relaxed text-muted">{tData(`${item.id}.description`)}</p>
                </div>
              </Reveal>
            );
          })}
